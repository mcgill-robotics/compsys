...
class Parser(object):
    ...
    def parse(self):
        ...
        parser.add_argument(
            "--version", action="version", version="v{}".format(self.version))

        # Add subparsers.
        subparsers = parser.add_subparsers(
            title="commands", help="valid subcommands")
        record_parser = subparsers.add_parser(
            "record",
            add_help=False,
            description=record.__doc__,
            formatter_class=argparse.RawDescriptionHelpFormatter,
            help="record topics into split bags")
        record_parser.set_defaults(cmd=record.Record)
        record_parser.add_argument(
            "--no-split", action="store_true", help="don't split bag")
        record_parser.add_argument(
            "--duration", default=15, help="seconds per split (default: 15)")
        record_parser.add_argument(
        ...
        for subparser in (record_parser, merge_parser):
            ...
            subparser.add_argument("--name", nargs=1, help="output name")
            subparser.add_argument(
                "dir",
                nargs='?',
                default='.',
        ...
        self.enabled = [
            ...
            if key != "cmd" and elem and type(elem) not in (int, bool, str,
                                                            list)
        ]
